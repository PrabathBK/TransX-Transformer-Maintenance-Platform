...
//   // CREATE — hits backend immediately (new box)
//   async function handleAnnotationCreate(bbox: {
//     x1: number;
//     y1: number;
//     x2: number;
//     y2: number;
...
//         // extended fields (persisted server-side)
//         annotationNumber: nextNumber,
//         sizePx,
...
//   // SAVE — persists annotated image AND current local annotation state
//   async function handleSaveAnnotatedImage() {
//     if (!inspection) {
//       alert('No inspection available');
//       return;
...
//       // Capture canvas to image and upload
//       const dataUrl = captureCanvasRef.current();
//       if (!dataUrl) {
//         alert('Failed to capture annotated image. Please ensure the image is fully loaded.');
//         return;
...
//       const file = new File(
//         [blob],
//         `annotated_${inspection.inspectionNumber}_${Date.now()}.png`,
//         { type: 'image/png' }
//       );
...
//       // Persist ALL current annotations (local state) to backend
//       for (const ann of annotations) {
//         await saveAnnotation({
//           id: ann.id, // if present, backend will version it
//           inspectionId: ann.inspectionId,
//           bbox: ann.bbox,
...
//           // extended fields
//           annotationNumber: (ann as any).annotationNumber,
//           sizePx: (ann as any).sizePx,
//           severityScore: (ann as any).severityScore,
//           flagged: (ann as any).flagged,
//         });
//       }
...
//             #{annotation.annotationNumber} · {annotation.className}
//           </div>
//           <div style={{ fontSize: '12px', color: '#6b7280', marginTop: '2px' }}>
//             {annotation.source === 'ai' ? '🤖 AI Detection' : '👤 Manual'}
//             {' · '}
...
//       {/* Delete allowed for both human AND approved */}
//       {(annotation.source === 'human' ) && (
//         <button
//           onClick={onDelete}
//           style={{
...
export default function InspectionDetailNew() {
  ...
  const [inspection, setInspection] = useState<Inspection | null>(null);
  const [annotations, setAnnotations] = useState<Annotation[]>([]); // session-only (sidebar)
  ...
  async function loadData() {
    ...
    try {
      ...
      // fetch inspection + annotations from backend
      const [inspectionData, annotationsData] = await Promise.all([
        getInspection(id),
        getAnnotationsByInspection(id),
      ]);
      ...
      setInspection(inspectionData);
      setAnnotationHistory(annotationsData); // <-- History list shows ALL saved annotations
      

      // Load baseline image for transformer
      if (inspectionData.transformerId) {
        await loadBaselineImage(
          inspectionData.transformerId,
          inspectionData.weatherCondition
  ...
  //     // Map backend detections → local annotation objects
  //     const detectedAnnotations: Annotation[] = (result.detections || []).map((det: any, idx: number) => ({
  //       id: crypto.randomUUID(), // temp ID
  //       inspectionId: inspection.id,
  //       bbox: det.bbox,
  //       classId: getClassId(det.className),
  ...
  //       sizePx: (det.bbox.x2 - det.bbox.x1) * (det.bbox.y2 - det.bbox.y1),
  //       severityScore: null,
  //       flagged: false,
  //       isTemporary: true,
  //     }));
...
// -------------------------
  async function handleDetectAnomalies() {
    ...
    try {
      ...
      // Start numbering AFTER any existing annotations
      const baseNumber = annotations.length;

      // Map backend detections → local annotation objects
      const detectedAnnotations: Annotation[] = (result.detections || []).map(
        (det: any, idx: number) => ({
          ...
          // 👇 Continuous numbering based on existing annotations
          annotationNumber: baseNumber + idx + 1,
      ...
      // Add new AI annotations while keeping existing manual ones
      setAnnotations((prev) => [...prev, ...detectedAnnotations]);
  ...
  // -------------------------
  // ✏️ Manual Annotation Create
  // -------------------------
  function handleAnnotationCreate(bbox: {
    x1: number;
    y1: number;
    x2: number;
    y2: number;
    ...
    // 👇 Continue numbering from all existing annotations
    const nextNumber = annotations.length + 1;
    ...
    const newAnnotation: Annotation & { isTemporary?: boolean } = {
      ...
      annotationNumber: nextNumber, // ✅ ensure sequential numbering
      sizePx,
      severityScore: null,
      flagged: false,
      isTemporary: true,
  ...
  function handleApprove(annotationId: string) {
    setAnnotations(prev =>
      prev.map(ann =>
        ann.id === annotationId
          ? { ...ann, actionType: "approved", isTemporary: true }
          : ann
      )
    );
  }
  
  function handleReject(annotationId: string) {
    // Simply remove it from current annotations list
    setAnnotations(prev => prev.filter(ann => ann.id !== annotationId));
  }
  ...
  async function handleSaveAnnotatedImage() {
    ...
    try {
      ...
      for (const ann of annotations) {
        await saveAnnotation({
          id: ann.isTemporary ? undefined : ann.id,
          inspectionId: ann.inspectionId,
          bbox: {
            x1: Math.round(ann.bbox.x1),
            y1: Math.round(ann.bbox.y1),
            x2: Math.round(ann.bbox.x2),
            y2: Math.round(ann.bbox.y2),
          },
          classId: ann.classId ?? getClassId(ann.className),
          className: ann.className,
          confidence: ann.confidence ?? 1.0,
          source: ann.source === "ai" ? "ai" : "human",
          actionType: "created",
          userId: ann.createdBy || "current-user@example.com",
          comments: ann.comments ?? "",
          annotationNumber: ann.annotationNumber ?? 0,
          sizePx: ann.sizePx ?? 0,
          severityScore: ann.severityScore ?? null,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '24px' }}>
        ...
        <div>
          ...
          <div style={{
            ...
            <HistoryList history={annotationHistory} />
          </div>
